...
# continuing code expansion using file version 4
...
while play_game:
    ...
    while guess != random_number:
        ...
        # end the game if invalid input is inputted. Use try and except for float, and isinstance for string
        try:
            # play or quit game
            if choice == 'q':
                play_game = False
                break
            elif 1 <= int(choice) <= 10:
                guess = int(choice)
                play_game = True
            elif isinstance(choice, str):  # isinstance(int(choice), int) is not needed b/c of conditional structure
                print("Invalid entry. Please enter a valid number between 1 and 10. Goodbye.")
                play_game = False
                break
        except ValueError:
            print("Invalid entry. Please enter a valid number between 1 and 10. Goodbye.")
